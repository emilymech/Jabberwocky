...
class Classifier:
    ...
    def create_model_config_files(self):
        ...
        with open(self.model_path+'/features.csv', 'w') as f:
    ...
    def compute_performance_summary(self):
        ...
        # recall, or true positive rate, or hit rate, % correct when correct=yes
        self.category_sensitivity_matrix = self.true_positives / (self.true_positives + self.false_negatives)

        # true negative rate, % correct when correct=no
        self.category_specificity_matrix = self.true_negatives / (self.true_negatives + self.false_positives)

        # % correct when you guessed yes
        self.category_precision_matrix = self.true_positives / (self.true_positives + self.false_positives)

        # % correct when you guessed no
        self.category_negative_predictive_value_matrix = self.true_negatives / \
                                                         (self.true_negatives + self.false_negatives)
        
        self.category_balanced_accuracy_matrix = (self.category_sensitivity_matrix +
                                                  self.category_specificity_matrix)/2
        self.category_f1_matrix = 2*self.true_positives / \
        ...
        self.negative_predictive_value = np.average(self.category_negative_predictive_value_matrix,
        ...
        if self.verbose:
            ...
            print("    negative predictive value {:0.3f}".format(self.negative_predictive_value),
                  self.category_negative_predictive_value_matrix)
            print("            balanced accuracy {:0.3f}".format(self.balanced_accuracy),
                  self.category_balanced_accuracy_matrix)
            print("                           f1 {:0.3f}".format(self.f1), self.category_f1_matrix)

    def save_performance_summary(self):
        with open(self.model_path+'/performance_summary.csv', 'w') as f:
            ...
            self.write_rate_performance_result(f, "negative_predictive_value", self.negative_predictive_value,
                                               self.category_negative_predictive_value_matrix)
            self.write_rate_performance_result(f, "balanced_accuracy", self.balanced_accuracy,
                                               self.category_balanced_accuracy_matrix)
    ...
    def calculate_and_save_current_epoch(self, i):
        ...
        if self.save_ba_history:
            with open(self.model_path+'/ba_history.csv', 'a') as f:
        ...
        if self.save_f1_history:
            with open(self.model_path+'/f1_history.csv', 'a') as f:
                vector_string = ",".join(np.char.mod('%0.3f', self.category_f1_matrix))
                f.write("{},{:0.3f},{}\n".format(i+1, self.f1, vector_string))

    def save_model(self):
        with open(self.model_path + '/model_object.p', 'wb') as pickle_file:
    ...
    def save_full_results(self):
        with open(self.model_path+'/instance_results.csv', 'w') as f:
